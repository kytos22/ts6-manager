...
import { Settings as SettingsIcon, Users, Server, Plus, Trash2, Pencil, TestTube, Check, X, Lock, KeyRound, Youtube, Upload, FileText, Monitor, MessagesSquare, Music, ShieldCheck, Copy } from 'lucide-react';
...
function SamlTab() {
  ...
  return (
    <Card>
      <CardHeader><CardTitle className="text-sm">{t('settings.tabs.saml')}</CardTitle></CardHeader>
      <CardContent className="space-y-4 max-w-xl">
        ...
        <div className="space-y-1.5">
          ...
          <Input className="h-8 text-xs" value={form.idpEntityId || ''} onChange={(e) => setForm((f) => ({ ...f, idpEntityId: e.target.value }))} />
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs">{t('settings.saml.idpSsoUrl')}</Label>
          <Input className="h-8 text-xs" value={form.idpSsoUrl || ''} onChange={(e) => setForm((f) => ({ ...f, idpSsoUrl: e.target.value }))} />
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs">{t('settings.saml.idpCertificate')}</Label>
          <textarea className="w-full h-24 rounded-md border border-border bg-background px-3 py-2 text-xs font-mono resize-none focus:outline-none focus:ring-1 focus:ring-ring"
            placeholder={settings.hasIdpCertificate ? '••••••••••••••••' : '-----BEGIN CERTIFICATE-----'} value={form.idpCertificate || ''}
            onChange={(e) => setForm((f) => ({ ...f, idpCertificate: e.target.value }))} />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label className="text-xs">{t('settings.saml.attrUsername')}</Label><Input className="h-8 text-xs" value={form.attrUsername || ''} onChange={(e) => setForm((f) => ({ ...f, attrUsername: e.target.value }))} /></div>
          <div className="space-y-1.5"><Label className="text-xs">{t('settings.saml.attrEmail')}</Label><Input className="h-8 text-xs" value={form.attrEmail || ''} onChange={(e) => setForm((f) => ({ ...f, attrEmail: e.target.value }))} /></div>
          <div className="space-y-1.5"><Label className="text-xs">{t('settings.saml.attrDisplayName')}</Label><Input className="h-8 text-xs" value={form.attrDisplayName || ''} onChange={(e) => setForm((f) => ({ ...f, attrDisplayName: e.target.value }))} /></div>
          <div className="space-y-1.5"><Label className="text-xs">{t('settings.saml.attrRole')}</Label><Input className="h-8 text-xs" value={form.attrRole || ''} onChange={(e) => setForm((f) => ({ ...f, attrRole: e.target.value || null }))} /></div>
        </div>

        <div className="space-y-1.5"><Label className="text-xs">{t('settings.saml.roleAdminValue')}</Label><Input className="h-8 text-xs" value={form.roleAdminValue || ''} onChange={(e) => setForm((f) => ({ ...f, roleAdminValue: e.target.value || null }))} /></div>
        <div className="flex items-center gap-2 pt-1"><Switch checked={!!form.autoProvision} onCheckedChange={(v) => setForm((f) => ({ ...f, autoProvision: v }))} /><Label className="text-xs">{t('settings.saml.autoProvision')}</Label></div>
        <div className="space-y-1.5">
          ...
          <Select value={form.defaultRole || 'viewer'} onValueChange={(v) => setForm((f) => ({ ...f, defaultRole: v as 'admin' | 'viewer' }))}>
            ...
            <SelectContent><SelectItem value="admin">{t('settings.users.roleAdmin')}</SelectItem><SelectItem value="viewer">{t('settings.users.roleViewer')}</SelectItem></SelectContent>
          </Select>
        </div>
        <Button size="sm" onClick={() => save.mutate()} disabled={save.isPending}>{save.isPending ? t('common.loading') : t('common.save')}</Button>
      </CardContent>
    </Card>
  );
}
